...
import { getWareHouse } from "../../services/contextGetCalls";
...
const Warehouse = () => {
	...
	const {
		...
		tableData: state.context.warehouse?.rows,
		dataCount: state.context.warehouse?.count,
	...
	const COLUMNS = useMemo(() => {
		return genrateColumnConfig(permissionData?.warehousePage?.Table1 || []);
	}, [loadingPermission]);
	//form default values
	const defaultValues = {
		label: "",
	...
	const handleChangeBulkDelete = async data => {
		...
		if (ids.length > 0) {
			const res = await changeBulkDelete(getURL("warehouse/bulk/delete"), {
				method: "DELETE",
				body: JSON.stringify({ ids: ids }),
			});
			if (res.status === 200) {
				message.success({
					message: translate("DeletewarehouseSuccessMessage"),
				});
				getWareHouse();
	...
	const handleChangeBulkHideShowAll = async (data, flag) => {
		...
		if (ids.length > 0) {
			const res = await changeBulkHideShow(
				getURL(`warehouse/${flag}/bulk/mark-visible`),
				{
					method: "PUT",
					body: JSON.stringify({ ids: ids }),
				}
			...
			if (res.status === 200) {
				flag
					? message.success({
							message: translate("ShowWarehouseSuccessMessage"),
					  })
					: message.success({
							message: translate("HideWarehouseSuccessMessage"),
					  });
	...
	const onSubmit = async value => {
		...
		const res = await createContext(getURL(`warehouse`), {
			method: "POST",
		...
		if (res && res.status === 201) {
			setOpened(false);
			message.success({
				message: translate("CreateWarehouseSuccessMessage"),
			});
			getWareHouse();
		}
		store.dispatch({
	...
	const buttonCofig = permissionData?.warehousePage?.topButtons;
		...
		<Container fluid>
			...
			<Modal
				...
				<ShowAndHideContextPopup
					...
					visible={false}
					heading={"Are you sure you want hide all Warehouse"}
				/>
			</Modal>
			...
			<Modal
				...
				<ShowAndHideContextPopup
					...
					handleChangeBulkHideShowAll={handleChangeBulkHideShowAll}
					visible={true}
					heading={"Are you sure you want show all Warehouse"}
				/>
			</Modal>
