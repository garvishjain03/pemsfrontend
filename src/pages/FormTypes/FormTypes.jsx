...
import { getFormTypes } from "../../services/contextGetCalls";
...
const FormTypes = () => {
	...
	const {
		...
		tableData: state.context.formtypes?.rows,
		dataCount: state.context.formtypes?.count,
	...
	const COLUMNS = useMemo(() => {
		return genrateColumnConfig(permissionData?.formTypesPage?.Table1 || []);
	}, [loadingPermission]);
	//form default values
	const defaultValues = {
		label: "",
	...
	const handleChangeBulkDelete = async data => {
		...
		if (ids.length > 0) {
			const res = await changeBulkDelete(getURL("formtypes/bulk/delete"), {
				method: "DELETE",
				body: JSON.stringify({ ids: ids }),
			});
			if (res.status === 200) {
				message.success({
					message: translate("DeleteformtypesSuccessMessage"),
				});
				getFormTypes();
	...
	const handleChangeBulkHideShowAll = async (data, flag) => {
		...
		if (ids.length > 0) {
			const res = await changeBulkHideShow(
				getURL(`formtypes/${flag}/bulk/changevisible`),
				{
					method: "PUT",
					body: JSON.stringify({ ids: ids }),
				}
			...
			if (res.status === 200) {
				flag
					? message.success({
							message: translate("ShowformtypesSuccessMessage"),
					  })
					: message.success({
							message: translate("HideformtypesSuccessMessage"),
					  });
	...
	const onSubmit = async value => {
		...
		const res = await createContext(getURL(`formtypes`), {
			method: "POST",
		...
		if (res && res.status === 201) {
			setOpened(false);
			message.success({
				message: translate("CreateformtypesSuccessMessage"),
			});
			getFormTypes();
		}
		store.dispatch({
	...
	const buttonCofig = permissionData?.formTypesPage?.topButtons;
		...
		<Container fluid>
			...
			<Modal
				...
				<ShowAndHideContextPopup
					...
					visible={false}
					heading={"Are you sure you want hide all Form types"}
				/>
			</Modal>
			...
			<Modal
				...
				<ShowAndHideContextPopup
					...
					handleChangeBulkHideShowAll={handleChangeBulkHideShowAll}
					visible={true}
					heading={"Are you sure you want show all Form types"}
				/>
			</Modal>
