...
import { getShapes } from "../../services/contextGetCalls";
...
const Shapes = () => {
	...
	const {
		...
		tableData: state.context.shapes?.rows,
		dataCount: state.context.shapes?.count,
	...
	const COLUMNS = useMemo(() => {
		return genrateColumnConfig(permissionData?.shapesPage?.Table1 || []);
	}, [loadingPermission]);
	//form default values
	const defaultValues = {
		label: "",
	...
	const handleChangeBulkDelete = async data => {
		...
		if (ids.length > 0) {
			const res = await changeBulkDelete(getURL("shapes/bulk/delete"), {
				method: "DELETE",
				body: JSON.stringify({ ids: ids }),
			});
			if (res.status === 200) {
				message.success({
					message: translate("DeleteshapesSuccessMessage"),
				});
				getShapes();
	...
	const handleChangeBulkHideShowAll = async (data, flag) => {
		...
		if (ids.length > 0) {
			const res = await changeBulkHideShow(
				getURL(`shapes/${flag}/bulk/mark-visible`),
				{
					method: "PUT",
					body: JSON.stringify({ ids: ids }),
				}
			...
			if (res.status === 200) {
				flag
					? message.success({
							message: translate("ShowshapesSuccessMessage"),
					  })
					: message.success({
							message: translate("HideshapesSuccessMessage"),
					  });
	...
	const onSubmit = async value => {
		...
		const res = await createContext(getURL(`shapes`), {
			method: "POST",
		...
		if (res && res.status === 201) {
			setOpened(false);
			message.success({
				message: translate("CreateshapesSuccessMessage"),
			});
			getShapes();
		}
		store.dispatch({
	...
	const buttonCofig = permissionData?.shapesPage?.topButtons;
		...
		<Container fluid>
			...
			<Modal
				...
				<ShowAndHideContextPopup
					...
					visible={false}
					heading={"Are you sure you want hide all Shape"}
				/>
			</Modal>
			...
			<Modal
				...
				<ShowAndHideContextPopup
					...
					handleChangeBulkHideShowAll={handleChangeBulkHideShowAll}
					visible={true}
					heading={"Are you sure you want show all Shape"}
				/>
			</Modal>
