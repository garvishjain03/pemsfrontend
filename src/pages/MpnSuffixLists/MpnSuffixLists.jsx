...
import { getMpnSuffixLists } from "../../services/contextGetCalls";
...
const MpnSuffxLists = () => {
	...
	const {
		...
		tableData: state.context.mpnsuffixlists?.rows,
		dataCount: state.context.mpnsuffixlists?.count,
	...
	const COLUMNS = useMemo(() => {
		return genrateColumnConfig(
			permissionData?.mpnSuffixListsPage?.Table1 || []
		);
	}, [loadingPermission]);
	//form default values
	const defaultValues = {
	...
	const handleChangeBulkDelete = async data => {
		...
		if (ids.length > 0) {
			const res = await changeBulkDelete(getURL("mpnsuffixlists/bulk/delete"), {
				method: "DELETE",
				body: JSON.stringify({ ids: ids }),
			});
			if (res.status === 200) {
				message.success({
					message: translate("DeletempnsuffixlistsSuccessMessage"),
				});
				getMpnSuffixLists();
	...
	const handleChangeBulkHideShowAll = async (data, flag) => {
		...
		if (ids.length > 0) {
			const res = await changeBulkHideShow(
				getURL(`mpnsuffixlists/${flag}/bulk/changevisible`),
				{
					method: "PUT",
					body: JSON.stringify({ ids: ids }),
				}
			...
			if (res.status === 200) {
				flag
					? message.success({
							message: translate("ShowmpnsuffixlistsSuccessMessage"),
					  })
					: message.success({
							message: translate("HidempnsuffixlistsSuccessMessage"),
					  });
	...
	const onSubmit = async value => {
		...
		const res = await createContext(getURL(`mpnsuffixlists`), {
			method: "POST",
		...
		if (res && res.status === 201) {
			setOpened(false);
			message.success({
				message: translate("CreatempnsuffixlistsSuccessMessage"),
			});
			getMpnSuffixLists();
		}
		store.dispatch({
	...
	const buttonCofig = permissionData?.mpnSuffixListsPage?.topButtons;
		...
		<Container fluid>
			...
			<Modal
				...
				<ShowAndHideContextPopup
					...
					visible={false}
					heading={"Are you sure you want hide all Mpn Suffix List"}
				/>
			</Modal>
			...
			<Modal
				...
				<ShowAndHideContextPopup
					...
					handleChangeBulkHideShowAll={handleChangeBulkHideShowAll}
					visible={true}
					heading={"Are you sure you want show all Mpn Suffix List"}
				/>
			</Modal>
