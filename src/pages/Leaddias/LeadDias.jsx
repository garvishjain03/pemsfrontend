...
import { getLeadDias } from "../../services/contextGetCalls";
...
const LeadDias = () => {
	...
	const {
		...
		tableData: state.context.leaddias?.rows,
		dataCount: state.context.leaddias?.count,
	...
	const COLUMNS = useMemo(() => {
		return genrateColumnConfig(permissionData?.leadDiaspage?.Table1 || []);
	}, [loadingPermission]);
	//form default values
	const defaultValues = {
		label: "",
	...
	const handleChangeBulkDelete = async data => {
		...
		if (ids.length > 0) {
			const res = await changeBulkDelete(getURL("leaddias/bulk/delete"), {
				method: "DELETE",
				body: JSON.stringify({ ids: ids }),
			});
			if (res.status === 200) {
				message.success({
					message: translate("DeleteleaddiasSuccessMessage"),
				});
				getLeadDias();
	...
	const handleChangeBulkHideShowAll = async (data, flag) => {
		...
		if (ids.length > 0) {
			const res = await changeBulkHideShow(
				getURL(`leaddias/${flag}/bulk/mark-visible`),
				{
					method: "PUT",
					body: JSON.stringify({ ids: ids }),
				}
			...
			if (res.status === 200) {
				flag
					? message.success({
							message: translate("ShowleaddiasSuccessMessage"),
					  })
					: message.success({
							message: translate("HideleaddiasSuccessMessage"),
					  });
	...
	const onSubmit = async value => {
		...
		const res = await createContext(getURL(`leaddias`), {
			method: "POST",
		...
		if (res && res.status === 201) {
			setOpened(false);
			message.success({
				message: translate("CreateleaddiasSuccessMessage"),
			});
			getLeadDias();
		}
		store.dispatch({
	...
	const buttonCofig = permissionData?.leadDiaspage?.topButtons;
		...
		<Container fluid>
			...
			<Modal
				...
				<ShowAndHideContextPopup
					...
					visible={false}
					heading={"Are you sure you want hide all Lead Dias"}
				/>
			</Modal>
			...
			<Modal
				...
				<ShowAndHideContextPopup
					...
					handleChangeBulkHideShowAll={handleChangeBulkHideShowAll}
					visible={true}
					heading={"Are you sure you want show all Lead Dias"}
				/>
			</Modal>
