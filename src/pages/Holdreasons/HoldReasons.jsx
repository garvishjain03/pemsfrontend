...
import { getHoldReasons } from "../../services/contextGetCalls";
...
const HoldReasons = () => {
	...
	const {
		...
		tableData: state.context.holdreasons?.rows,
		dataCount: state.context.holdreasons?.count,
	...
	const COLUMNS = useMemo(() => {
		return genrateColumnConfig(permissionData?.holdReasonsPage?.Table1 || []);
	}, [loadingPermission]);
	//form default values
	const defaultValues = {
		label: "",
	...
	const handleChangeBulkDelete = async data => {
		...
		if (ids.length > 0) {
			const res = await changeBulkDelete(getURL("holdreasons/bulk/delete"), {
				method: "DELETE",
				body: JSON.stringify({ ids: ids }),
			});
			if (res.status === 200) {
				message.success({
					message: translate("DeleteholdreasonsSuccessMessage"),
				});
				getHoldReasons();
	...
	const handleChangeBulkHideShowAll = async (data, flag) => {
		...
		if (ids.length > 0) {
			const res = await changeBulkHideShow(
				getURL(`holdreasons/${flag}/bulk/changevisible`),
				{
					method: "PUT",
					body: JSON.stringify({ ids: ids }),
				}
			...
			if (res.status === 200) {
				flag
					? message.success({
							message: translate("ShowholdreasonsSuccessMessage"),
					  })
					: message.success({
							message: translate("HideholdreasonsSuccessMessage"),
					  });
	...
	const onSubmit = async value => {
		...
		const res = await createContext(getURL(`holdreasons`), {
			method: "POST",
		...
		if (res && res.status === 201) {
			setOpened(false);
			message.success({
				message: translate("CreateholdreasonsSuccessMessage"),
			});
			getHoldReasons();
		}
		store.dispatch({
	...
	const buttonCofig = permissionData?.holdReasonsPage?.topButtons;
		...
		<Container fluid>
			...
			<Modal
				...
				<ShowAndHideContextPopup
					...
					visible={false}
					heading={"Are you sure you want hide all Hold Reason"}
				/>
			</Modal>
			...
			<Modal
				...
				<ShowAndHideContextPopup
					...
					handleChangeBulkHideShowAll={handleChangeBulkHideShowAll}
					visible={true}
					heading={"Are you sure you want show all Hold Reason"}
				/>
			</Modal>
