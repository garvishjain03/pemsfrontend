...
import { getCharacteristics } from "../../services/contextGetCalls";
...
const Characteristics = () => {
	...
	const {
		...
		tableData: state.context.characteristics?.rows,
		dataCount: state.context.characteristics?.count,
	...
	const COLUMNS = useMemo(() => {
		return genrateColumnConfig(
			permissionData?.characteristicsPage?.Table1 || []
		);
	}, [loadingPermission]);
	//form default values
	const defaultValues = {
	...
	const handleChangeBulkDelete = async data => {
		...
		if (ids.length > 0) {
			const res = await changeBulkDelete(
				getURL("characteristics/bulk/delete"),
				{
					method: "DELETE",
			...
			if (res.status === 200) {
				message.success({
					message: translate("DeletecharacteristicsSuccessMessage"),
				});
				getCharacteristics();
	...
	const handleChangeBulkHideShowAll = async (data, flag) => {
		...
		if (ids.length > 0) {
			const res = await changeBulkHideShow(
				getURL(`characteristics/${flag}/bulk/mark-visible`),
				{
					method: "PUT",
					body: JSON.stringify({ ids: ids }),
				}
			...
			if (res.status === 200) {
				flag
					? message.success({
							message: translate("ShowCharacteristicsSucessMessage"),
					  })
					: message.success({
							message: translate("HideCharacteristicsSucessMessage"),
					  });
	...
	const onSubmit = async value => {
		...
		const res = await createContext(getURL(`characteristics`), {
			method: "POST",
		...
		if (res && res.status === 201) {
			setOpened(false);
			message.success({
				message: translate("CreateCharacteristicsSuccessMessage"),
			});
			getCharacteristics();
		}
		store.dispatch({
	...
	const buttonCofig = permissionData?.characteristicsPage?.topButtons;
		...
		<Container fluid>
			...
			<Modal
				...
				<ShowAndHideContextPopup
					...
					visible={false}
					heading={"Are you sure you want hide all Characteristics"}
				/>
			</Modal>
			...
			<Modal
				...
				<ShowAndHideContextPopup
					...
					handleChangeBulkHideShowAll={handleChangeBulkHideShowAll}
					visible={true}
					heading={"Are you sure you want show all Characteristics"}
				/>
			</Modal>
