...
import { getCoreSizes } from "../../services/contextGetCalls";
...
const CoreSizes = () => {
	...
	const {
		...
		tableData: state.context.coresizes?.rows,
		dataCount: state.context.coresizes?.count,
	...
	const COLUMNS = useMemo(() => {
		return genrateColumnConfig(permissionData?.coreSizePage?.Table1 || []);
	}, [loadingPermission]);
	//form default values
	const defaultValues = {
		label: "",
	...
	const handleChangeBulkDelete = async data => {
		...
		if (ids.length > 0) {
			const res = await changeBulkDelete(getURL("coresizes/bulk/delete"), {
				method: "DELETE",
				body: JSON.stringify({ ids: ids }),
			});
			if (res.status === 200) {
				message.success({
					message: translate("DeletecoreizesSuccessMessage"),
				});
				getCoreSizes();
	...
	const handleChangeBulkHideShowAll = async (data, flag) => {
		...
		if (ids.length > 0) {
			const res = await changeBulkHideShow(
				getURL(`coresizes/${flag}/bulk/changevisible`),
				{
					method: "PUT",
					body: JSON.stringify({ ids: ids }),
				}
			...
			if (res.status === 200) {
				flag
					? message.success({
							message: translate("ShowcoresizesSuccessMessage"),
					  })
					: message.success({
							message: translate("HidecoresizesSuccessMessage"),
					  });
	...
	const onSubmit = async value => {
		...
		const res = await createContext(getURL(`coresizes`), {
			method: "POST",
		...
		if (res && res.status === 201) {
			setOpened(false);
			message.success({
				message: translate("CreatecoresizesSuccessMessage"),
			});
			getCoreSizes();
		}
		store.dispatch({
	...
	const buttonCofig = permissionData?.coreSizePage?.topButtons;
		...
		<Container fluid>
			...
			<Modal
				...
				<ShowAndHideContextPopup
					...
					visible={false}
					heading={"Are you sure you want hide all CoreSize"}
				/>
			</Modal>
			...
			<Modal
				...
				<ShowAndHideContextPopup
					...
					handleChangeBulkHideShowAll={handleChangeBulkHideShowAll}
					visible={true}
					heading={"Are you sure you want show all CoreSize"}
				/>
			</Modal>
