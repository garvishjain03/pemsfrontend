...
import { getDelayReasons } from "../../services/contextGetCalls";
...
const DelayReason = () => {
	...
	const {
		...
		tableData: state.context.delayReason?.rows,
		dataCount: state.context.delayReason?.count,
	...
	const COLUMNS = useMemo(() => {
		return genrateColumnConfig(permissionData?.delayReasonsPage?.Table1 || []);
	}, [loadingPermission]);
	...
	const handleChangeBulkDelete = async data => {
		...
		if (ids.length > 0) {
			const res = await changeBulkDelete(getURL("delayreasons/bulk/delete"), {
				method: "DELETE",
				body: JSON.stringify({ ids: ids }),
			});
			if (res.status === 200) {
				message.success({
					message: translate("DeletedelayreasonsSuccessMessage"),
				});
				getDelayReasons();
	...
	const handleChangeBulkHideShowAll = async (data, flag) => {
		...
		if (ids.length > 0) {
			const res = await changeBulkHideShow(
				getURL(`delayreasons/${flag}/bulk/changevisible`),
				{
					method: "PUT",
					body: JSON.stringify({ ids: ids }),
				}
			...
			if (res.status === 200) {
				flag
					? message.success({
							message: translate("ShowDelayReasonsSucessMessage"),
					  })
					: message.success({
							message: translate("HideDelayReasonsSucessMessage"),
					  });
	...
	const onSubmit = async value => {
		...
		const res = await createContext(getURL(`delayreasons`), {
			method: "POST",
		...
		if (res && res.status === 201) {
			setOpened(false);
			message.success({
				message: translate("CreateDelayReasonsSuccessMessage"),
			});
			getDelayReasons();
		}
		store.dispatch({
	...
	const buttonCofig = permissionData?.delayReasonsPage?.topButtons;
		...
		<Container fluid>
			...
			<Modal
				...
				<ShowAndHideContextPopup
					...
					visible={false}
					heading={"Are you sure you want hide all Delay Reason"}
				/>
			</Modal>
			<Modal
				centered
				...
				<ShowAndHideContextPopup
					...
					handleChangeBulkHideShowAll={handleChangeBulkHideShowAll}
					visible={true}
					heading={"Are you sure you want show all Delay Reason"}
				/>
			</Modal>
			<Modal
				centered
