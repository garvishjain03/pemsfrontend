...
import { getTypes } from "../../services/contextGetCalls";
...
const Types = () => {
	...
	const {
		...
		tableData: state.context.types?.rows,
		dataCount: state.context.types?.count,
	...
	const COLUMNS = useMemo(() => {
		return genrateColumnConfig(permissionData?.typesPage?.Table1 || []);
	}, [loadingPermission]);
	//form default values
	const defaultValues = {
		label: "",
	...
	const handleChangeBulkDelete = async data => {
		...
		if (ids.length > 0) {
			const res = await changeBulkDelete(getURL("types/bulk/delete"), {
				method: "DELETE",
				body: JSON.stringify({ ids: ids }),
			});
			if (res.status === 200) {
				message.success({
					message: translate("DeletetypesSuccessMessage"),
				});
				getTypes();
	...
	const handleChangeBulkHideShowAll = async (data, flag) => {
		...
		if (ids.length > 0) {
			const res = await changeBulkHideShow(
				getURL(`types/${flag}/bulk/mark-visible`),
				{
					method: "PUT",
					body: JSON.stringify({ ids: ids }),
				}
			...
			if (res.status === 200) {
				flag
					? message.success({
							message: translate("ShowTypesSuccessMessage"),
					  })
					: message.success({
							message: translate("HideTypesSuccessMessage"),
					  });
	...
	const onSubmit = async value => {
		...
		const res = await createContext(getURL(`types`), {
			method: "POST",
		...
		if (res && res.status === 201) {
			setOpened(false);
			message.success({
				message: translate("CreateTypesSuccessMessage"),
			});
			getTypes();
		}
		store.dispatch({
	...
	const buttonCofig = permissionData?.typesPage?.topButtons;
		...
		<Container fluid>
			...
			<Modal
				...
				<ShowAndHideContextPopup
					...
					handleChangeBulkHideShowAll={handleChangeBulkHideShowAll}
					visible={true}
					heading={"Are you sure you want show all Types"}
				/>
			</Modal>
				...
				<Group position="right" spacing={4}>
					...
					{showbutton("showAllButton") ? (
						<Button
							color={"green"}
							disabled={
								selected?.filter(row => row.visible === false).length === 0
							}
					...
					{showbutton("hideAllButton") ? (
						<Button
							color="red"
							disabled={
								selected?.filter(row => row.visible === true).length === 0
							}
