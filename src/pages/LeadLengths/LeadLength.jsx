...
import { getLeadLengths } from "../../services/contextGetCalls";
...
const LeadLengths = () => {
	...
	const {
		...
		tableData: state.context.leadlengths?.rows,
		dataCount: state.context.leadlengths?.count,
	...
	const COLUMNS = useMemo(() => {
		return genrateColumnConfig(permissionData?.leadLengthPage?.Table1 || []);
	}, [loadingPermission]);
	//form default values
	const defaultValues = {
		label: "",
	...
	const handleChangeBulkDelete = async data => {
		...
		if (ids.length > 0) {
			const res = await changeBulkDelete(getURL("leadlengths/bulk/delete"), {
				method: "DELETE",
				body: JSON.stringify({ ids: ids }),
			});
			if (res.status === 200) {
				message.success({
					message: translate("DeleteleadlengthsSuccessMessage"),
				});
				getLeadLengths();
	...
	const handleChangeBulkHideShowAll = async (data, flag) => {
		...
		if (ids.length > 0) {
			const res = await changeBulkHideShow(
				getURL(`leadlengths/${flag}/bulk/mark-visible`),
				{
					method: "PUT",
					body: JSON.stringify({ ids: ids }),
				}
			...
			if (res.status === 200) {
				flag
					? message.success({
							message: translate("ShowleadlengthsSuccessMessage"),
					  })
					: message.success({
							message: translate("Hideleadlengths	SuccessMessage"),
					  });
	...
	const onSubmit = async value => {
		...
		const res = await createContext(getURL(`leadlengths`), {
			method: "POST",
		...
		if (res && res.status === 201) {
			setOpened(false);
			message.success({
				message: translate("CreateleadlengthsSuccessMessage"),
			});
			getLeadLengths();
		}
		store.dispatch({
	...
	const buttonCofig = permissionData?.leadLengthPage?.topButtons;
		...
		<Container fluid>
			...
			<Modal
				...
				<ShowAndHideContextPopup
					...
					visible={false}
					heading={"Are you sure you want show all Lead Length"}
				/>
			</Modal>
