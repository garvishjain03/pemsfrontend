...
import { getTcrs } from "../../services/contextGetCalls";
...
const Tcrs = () => {
	...
	const {
		...
		tableData: state.context.tcrs?.rows,
		dataCount: state.context.tcrs?.count,
	...
	const COLUMNS = useMemo(() => {
		return genrateColumnConfig(permissionData?.tcrsPage?.Table1 || []);
	}, [loadingPermission]);
	//form default values
	const defaultValues = {
		label: "",
	...
	const handleChangeBulkDelete = async data => {
		...
		if (ids.length > 0) {
			const res = await changeBulkDelete(getURL("tcrs/bulk/delete"), {
				method: "DELETE",
				body: JSON.stringify({ ids: ids }),
			});
			if (res.status === 200) {
				message.success({
					message: translate("DeletetcrsSuccessMessage"),
				});
				getTcrs();
	...
	const handleChangeBulkHideShowAll = async (data, flag) => {
		...
		if (ids.length > 0) {
			const res = await changeBulkHideShow(
				getURL(`tcrs/${flag}/bulk/mark-visible`),
				{
					method: "PUT",
					body: JSON.stringify({ ids: ids }),
				}
			...
			if (res.status === 200) {
				flag
					? message.success({
							message: translate("ShowtcrsSuccessMessage"),
					  })
					: message.success({
							message: translate("HidetcrsSuccessMessage"),
					  });
	...
	const onSubmit = async value => {
		...
		const res = await createContext(getURL(`tcrs`), {
			method: "POST",
		...
		if (res && res.status === 201) {
			setOpened(false);
			message.success({
				message: translate("CreatetcrsSuccessMessage"),
			});
			getTcrs();
		}
		store.dispatch({
	...
	const buttonCofig = permissionData?.tcrsPage?.topButtons;
		...
		<Container fluid>
			...
			<Modal
				...
				<ShowAndHideContextPopup
					...
					visible={false}
					heading={"Are you sure you want hide all Tcrs"}
				/>
			</Modal>
			...
			<Modal
				...
				<ShowAndHideContextPopup
					...
					handleChangeBulkHideShowAll={handleChangeBulkHideShowAll}
					visible={true}
					heading={"Are you sure you want show all Tcrs"}
				/>
			</Modal>
